...
export default async function FotografiasPage({ params }: PageProps<"/pacientes/[id]/fotografias">) {
  ...
  return (
    <div className="space-y-6">
      <form action={accion} className="grid grid-cols-2 gap-3 rounded-lg border border-stone-200 bg-white p-4 sm:grid-cols-5">
        <input type="file" name="archivo" accept="image/*" required className="col-span-2 text-sm sm:col-span-1" />
        <select name="vista" className="rounded-md border border-stone-300 px-2 py-1.5 text-sm">
          <option value="frente">Frente</option>
          <option value="perfil">Perfil</option>
          <option value="espalda">Espalda</option>
          <option value="otra">Otra</option>
        </select>
        <input type="date" name="fecha_captura" required className="rounded-md border border-stone-300 px-2 py-1.5 text-sm" />
        <input type="text" name="observaciones" placeholder="Observaciones" className="rounded-md border border-stone-300 px-2 py-1.5 text-sm" />
      ...
      {fotosConUrl.length === 0 ? (
        <p className="text-sm text-stone-500">Sin fotografías de progreso todavía.</p>
      ) : (
